...
const initialStates = {isLoginMode: true,email: '',password: '',confirmPassword: '',babyBirthday: '',inviteCode: '',errorMessage: '',successMessage: '',isLoading: false,isLoggedIn: false,userEmail: '',isMember: false}
Object.keys(initialStates).forEach(key => {window[key] = ref(initialStates[key])})
onMounted(() => {
  const storedEmail = localStorage.getItem('soothe_email')
  if (storedEmail) {isLoggedIn.value = true;userEmail.value = storedEmail;isMember.value = localStorage.getItem('soothe_is_member') === 'true'}
})
const switchMode = () => {isLoginMode.value = !isLoginMode.value,errorMessage.value = '',successMessage.value = '',password.value = '',confirmPassword.value = '',babyBirthday.value = '',inviteCode.value = ''}
const submitAuth = async () => {
  errorMessage.value = '';successMessage.value = ''
  if (!email.value || !password.value) {errorMessage.value = '邮箱和密码不能为空！';return}
  ...
  if (!isLoginMode.value) {
    ...
    if (!babyBirthday.value) {errorMessage.value = '请选择宝宝生日，方便我们提供精准服务！';return}
  }
  ...
  try {
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({email: email.value,password: password.value,baby_birthday: isLoginMode.value ? "" : babyBirthday.value,yaoQinMa: isLoginMode.value ? "" : inviteCode.value,souJiHao: "",display_name: "网页用户",gender: "",nation: ""})
    })
    ...
    if (!response.ok) {
      ...
      } else {errorMessage.value = backendError}
    } else {if (isLoginMode.value) {localStorage.setItem('soothe_uid', data.uid);localStorage.setItem('soothe_email', data.email);localStorage.setItem('soothe_is_member', data.is_member);isLoggedIn.value = true;userEmail.value = data.email;isMember.value = data.is_member} else {successMessage.value = '注册成功！请登录。';isLoginMode.value = true;password.value = '';confirmPassword.value = ''}}
  } catch (error) {errorMessage.value = '服务器开了小差或网络中断，请稍后再试。'} finally {isLoading.value = false}
}
const logout = () => {localStorage.removeItem('soothe_uid');localStorage.removeItem('soothe_email');localStorage.removeItem('soothe_is_member');isLoggedIn.value = false;email.value = '';password.value = ''}
</script>
